...
public class TC_005_DoctorModule {
	@Test
	...
	public void PatientprofileUpdate() throws AWTException {
		...
		DoctorHomePag.getPatientElement().click();
		DoctorHomePag.getAddPatientElement().click();
		sa.assertEquals(driver.getTitle(), "Doctor | Add Patient", "Add Patient Page is not Displayed ");
		Reporter.log("Add Patient Page is Displayed", true);

		DoctorHomePag.getAddName().sendKeys("Nagendra");
		DoctorHomePag.getAddContact().sendKeys("1234567892");
		DoctorHomePag.getAddEmail().sendKeys("noemail1@gmail.com");
		DoctorHomePag.getAddGender().click();
		DoctorHomePag.getAddAddress().sendKeys("Gopaln Coworks");
		DoctorHomePag.getAddAge().sendKeys("25");
		DoctorHomePag.getMedicalHistoryElement().sendKeys("Everythis is good");
		DoctorHomePag.getAddButton().click();
